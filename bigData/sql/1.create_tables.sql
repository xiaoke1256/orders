...
create table BIG_DATA_MODEL(
  ...
  class_define varchar(1024) comment '分类定义（聚集算法用），即分类的label和分类的名称的对应关系（废除）',
  create_time DATETIME not null DEFAULT NOW() comment '创建时间',
  create_by VARCHAR(32) comment '创建人'
)comment = '大数据模型' ENGINE=InnoDB CHARSET=utf8mb4;

-- 聚类定义
create table BIG_DATA_CLUSTER_DEFINED(
...
)comment = '聚类定义' ENGINE=InnoDB CHARSET=utf8mb4;
...
)comment = '计算执行信息' ENGINE=InnoDB CHARSET=utf8mb4;
...
create table BIG_DATA_CLUSTER_OBJECT_MAP(
   ...
   label varchar(10) not null comment '（算法产生的）标签',
   object_type char(2) comment '业务对象类型：01：商品，02：用户',
   object_id varchar(64) not null comment '业务对象主键',
   insert_time DATETIME not null DEFAULT NOW() comment '创建时间'
)comment = '聚类与对象的映射' ENGINE=InnoDB CHARSET=utf8mb4;