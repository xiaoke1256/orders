...
class ProductClusterServiceKmeansImpl {
   ...
   */
  def trainModel(productList:java.util.List[SimpleProductStatic],numClusters: Int,numIterator:Int,modelPath:String
    ...
    val parsedData = sc.parallelize[SimpleProductStatic](productListSeq)
      ...
      .map(p=>Vectors.dense(p.getProductPrice/1000.0*productPriceCoefficient,p.getOrderCount.toDouble*orderCountCoefficient)).cache()
...
//      }
   ...
   * @return
   */
  def predict(modelPath:String,productList:java.util.List[SimpleProductStatic]
              ,productPriceCoefficient:Double,orderCountCoefficient:Double): java.util.List[PredictResult[SimpleProductStatic]] = {
    val sc = sparkSession.sparkContext
    val model = KMeansModel.load(sc,modelPath)

    val productListSeq = JavaConverters.asScalaIteratorConverter(productList.iterator()).asScala.toSeq//把List转成Seq

    val result = sc.parallelize(productListSeq).filter((p)=>p.getOrderCount>0).map((p)=>{
      val label = model.predict(Vectors.dense(p.getProductPrice.toDouble/1000.0*productPriceCoefficient,p.getOrderCount.toDouble*orderCountCoefficient))
