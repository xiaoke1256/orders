...
(
  ...
  default_acc_no VARCHAR(18) not null comment '商户收款账号',
  public_key VARCHAR(1024) not null comment '商户公钥',
  insert_time TIMESTAMP not null DEFAULT NOW() comment '插入时间',
  update_time TIMESTAMP not null DEFAULT NOW() comment '修改时间'
) comment='商户表';
...
(merchant_no,merchant_name,default_acc_no,public_key,insert_time,update_time)
